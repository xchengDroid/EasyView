# EasyPermission
一款非常简洁，依赖很少的权限请求框架。不依赖第三方类库，如RxJava，不依赖运行时或编译时注解，且**无须在Activity和Fragment发起请求和接收返回**。核心代码只有5个类,并且适配了小米 AppOps 权限，。内部还封装了一些View框架，EasyActivity,EasyFrament，如果不需要可以仅仅将权限部分到自己的代码中。
