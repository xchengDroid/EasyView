...
public class ProgressView extends View {
    ...
    public void setup(Context context, AttributeSet attrs) {
        ...
        mType = array.getInt(R.styleable.ProgressView_ev_type, TYPE_RECT);
        mProgressColor = array.getColor(R.styleable.ProgressView_ev_progress_color, DEFAULT_PROGRESS_COLOR);
        mBackgroundColor = array.getColor(R.styleable.ProgressView_ev_background_color, DEFAULT_BACKGROUND_COLOR);

        mMaxValue = array.getInt(R.styleable.ProgressView_ev_max_value, 100);
        mValue = array.getInt(R.styleable.ProgressView_ev_value, 0);

        boolean isRoundCap = array.getBoolean(R.styleable.ProgressView_ev_stroke_round_cap, false);

        int textSize = DEFAULT_TEXT_SIZE;
        if (array.hasValue(R.styleable.ProgressView_android_textSize)) {
            textSize = array.getDimensionPixelSize(R.styleable.ProgressView_android_textSize, DEFAULT_TEXT_SIZE);
        }
        int textColor = DEFAULT_TEXT_COLOR;
        if (array.hasValue(R.styleable.ProgressView_android_textColor)) {
            textColor = array.getColor(R.styleable.ProgressView_android_textColor, DEFAULT_TEXT_COLOR);
        }

        if (mType == TYPE_CIRCLE) {
            mStrokeWidth = array.getDimensionPixelSize(R.styleable.ProgressView_ev_stroke_width, DEFAULT_STROKE_WIDTH);
        }
        array.recycle();
    ...
    public interface QMUIProgressBarTextGenerator {
         ...
         * @return 进度文案
         */
        String generateText(ProgressView progressBar, int value, int maxValue);
    }
}
