 ...
 */
package com.xcheng.view.widget.smarttab;
...
public class SmartTabLayout extends HorizontalScrollView {
    ...
    protected void onLayout(boolean changed, int l, int t, int r, int b) {
        ...
        if (tabStrip.isIndicatorAlwaysInCenter() && tabStrip.getChildCount() > 0) {
            ...
            int start = (getMeasuredWidth() - Utils.getMeasuredWidth(firstTab)) / 2 - Utils.getMarginStart(firstTab);
            int end = (getMeasuredWidth() - Utils.getMeasuredWidth(lastTab)) / 2 - Utils.getMarginEnd(lastTab);
            if (ViewCompat.getPaddingStart(this) != start || ViewCompat.getPaddingEnd(this) != end) {
    ...
    public void setViewPager(ViewPager viewPager) {
        ...
        if (viewPager != null && viewPager.getAdapter() != null) {
            //防止多次调用setViewPager方法重复添加，因为我们无法通过viewPager判断是否已经添加过
            viewPager.removeOnPageChangeListener(internalViewPagerListener);
            viewPager.addOnPageChangeListener(internalViewPagerListener);
            populateTabStrip();
        }
    }

    private InternalViewPagerListener internalViewPagerListener = new InternalViewPagerListener();
